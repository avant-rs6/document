...
class Extractor
{
    ...
    public static function getClassMethodAnnotations($className)
    {
        ...
        foreach ($class->getMethods() as $object) {
            ...
            $fun = self::getMethodAnnotations($className, $object->name);

        // if ($className == "app\index\controller\Worker") {
        //     var_dump($fun);die;
        // }

            if (empty($fun['ApiTitle']) || empty(reset($fun['ApiTitle']))) {
                self::$classMethodAnnotationCache[$className][$object->name] = [];

                continue;
            }

            self::$classMethodAnnotationCache[$className][$object->name] = $fun;
        }
    ...
    private static function consolidateAnnotations($method, $class)
    {
        ...
        $methodAnnotations = self::parseAnnotations($docblockMethod, true);
        // var_dump($methodAnnotations);die;
     ...
     * @return array  parsed annotations params
     */
    private static function parseAnnotations($docblock, $type = false)
    {
        $annotations = array();
        ...
        if (!isset($annotations['ApiTitle'])) {
            preg_match_all("/@(ApiTitle)[\s\t]*\((?<args>(?:(?!\)).)*)\)\r?/s", str_replace('/**', '', $docblock), $matchArr);
